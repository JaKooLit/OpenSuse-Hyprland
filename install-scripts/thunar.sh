...
printf "${NOTE} Installing additional Thunar Packages...\n"
  for THUNAR in "${thunar[@]}"; do
    ...
    [ $? -ne 0 ] && { echo -e "\e[1A\e[K${ERROR} - $THUNAR has failed to install, please check install.log"; exit 1; }
  done
...
for DIR1 in Thunar xfce4; do
  ...
  if [ -d "$DIRPATH" ]; then
    ...
    cp -r assets/$DIR1 ~/.config/ && echo "Copying to $DIR1 completed!" || echo "Error: Failed to copy config files to $DIR1." 2>&1 | tee -a "$LOG"
  fi
done
